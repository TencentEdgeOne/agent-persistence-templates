 ...
 *   context.store           — store adapter (appendMessage / getMessages / claudeSessionStore / claudeSessionBinding)
...
async function resolveClaudeSessionBinding(
  ...
  // New runtimes map arbitrary Conversation IDs to a stable Claude session ID.
  // This must be the primary path: Conversation IDs are not required to be UUIDs.
  if (typeof store?.claudeSessionBinding === 'function') {
    try {
      const binding = await store.claudeSessionBinding(conversationId);
      const sessionId = readSessionId(binding);
      if (sessionId) {
        // `dir` is load-bearing: getSessionInfo derives the project key from it,
        // while SessionStore namespaces its blob keys by projectKey.
        try {
          ...
          logger.error('[session] failed to inspect bound session for resume:', e);
        }
        return { sessionId };
      }
      logger.log(`[session] runtime returned no Claude session binding for conversationId=${conversationId}`);
    } catch (e) {
      logger.error('[session] failed to resolve runtime Claude session binding:', e);
    }
  } else {
    // Compatibility path for runtimes predating claudeSessionBinding(). It is
    // intentionally UUID-only; arbitrary Conversation IDs require the new API.
    const legacySessionId = normalizeUuid(conversationId);
    if (!legacySessionId) {
      logger.log(`[session] skip legacy SDK session binding: invalid conversationId=${conversationId}`);
      return {};
    }

    try {
      const info = await getSessionInfo(legacySessionId, { dir: cwd, sessionStore });
      if (info) {
        logger.log(`[session] resume legacy Claude SDK sessionId=${legacySessionId}`);
        return { resume: legacySessionId };
      }
      logger.log(`[session] create legacy Claude SDK sessionId=${legacySessionId}`);
    } catch (e) {
      logger.error('[session] failed to inspect legacy sessionStore for resume:', e);
    }
...
export async function onRequest(context: any) {
  ...
  const sessionBinding = await resolveClaudeSessionBinding(store, claudeSessionStore, conversationId, process.cwd());
